...
class ShareHumanoid(BaseTask):
    def __init__(self, cfg, sim_params, physics_engine, device_type, device_id, headless):
        ...
        self.humanoid_number = cfg["env"]["numHumanoids"]
        ...
        sensors_per_env = self.vec_sensor_tensor.shape[0] // self.num_envs
        self.vec_sensor_tensor = self.vec_sensor_tensor.view(
        ...
        self._humanoid_actor_ids = num_actors * \
            torch.arange(self.num_envs, device=self.device, dtype=torch.int32)

        # create some wrapper tensors for different slices
        self._dof_state = gymtorch.wrap_tensor(dof_state_tensor)
        dofs_per_env = self._dof_state.shape[0] // self.num_envs
        self._dof_pos = self._dof_state.view(
            self.num_envs, dofs_per_env, 2)[..., :self.num_dof, 0]
        self._dof_vel = self._dof_state.view(
            self.num_envs, dofs_per_env, 2)[..., :self.num_dof, 1]
        ...
        self._rigid_body_pos = rigid_body_state_reshaped[...,
                                                         :self.num_bodies, 0:3]
        self._rigid_body_rot = rigid_body_state_reshaped[...,
                                                         :self.num_bodies, 3:7]
        self._rigid_body_vel = rigid_body_state_reshaped[...,
        ...
        self.contact_bodies = self.cfg["env"]["contactBodies"]
        self.contact_objects = self.cfg["env"]["contactObjects"]
        self._key_body_ids = self._build_key_body_ids_tensor(key_bodies)
        self._contact_body_ids = self._build_contact_body_ids_tensor(
            self.contact_bodies)
        self._contact_object_ids = self._build_contact_object_ids_tensor(
    ...
    def _setup_character_props(self, key_bodies):
        ...
        if (asset_file == "mjcf/amp_humanoid.xml"):
            ...
            self._dof_offsets = [0, 3, 6, 9, 10,
                                 13, 14, 17, 18, 21, 24, 25, 28]
    ...
    def _build_termination_heights(self):
        ...
        self._termination_heights = np.array(
            [termination_height] * self.num_bodies)
    ...
    def _build_env(self, env_id, env_ptr, humanoid_asset):
        ...
        if self.humanoid_number == 4:
            if env_id in self.base_env_ids:
                col_group = torch.where(env_id == self.base_env_ids)[0].item()
            else:
                for offset_env_id in self.offset_env_ids_list:
                    if env_id in offset_env_id:
                        col_group = torch.where(
                            env_id == offset_env_id)[0].item()
                        break
        elif self.humanoid_number == 2:
        ...
        for j in range(self.num_bodies):
            self.gym.set_rigid_body_color(
                env_ptr, humanoid_handle, j, gymapi.MESH_VISUAL, gymapi.Vec3(0.54, 0.85, 0.2))

        if (self._pd_control):
            dof_prop = self.gym.get_asset_dof_properties(humanoid_asset)
            dof_prop["driveMode"] = gymapi.DOF_MODE_POS
            self.gym.set_actor_dof_properties(
                env_ptr, humanoid_handle, dof_prop)
    ...
    def _build_contact_object_ids_tensor(self, contact_object_names):
        env_ptr = self.envs[0]
        # TODO: hard code
        # object_handle_list = [self._box_handles[0],
        #                       self._marker_handles[0], self._platform_handles[0]]
        object_handle_list = [self._box_handles[0]]
        object_ids = []

        for body_name in contact_object_names:
            for obj_handle in object_handle_list:
                obj_id = self.gym.find_actor_rigid_body_handle(
                    env_ptr, obj_handle, body_name)
                if obj_id != -1:
                    object_ids.append(obj_id)
                    break

        object_ids = to_torch(object_ids, device=self.device, dtype=torch.long)
        return object_ids
    ...
    def _init_camera(self):
        self.gym.refresh_actor_root_state_tensor(self.sim)
        self._cam_prev_char_pos = self._humanoid_root_states[0, 0:3].cpu(
        ).numpy()
